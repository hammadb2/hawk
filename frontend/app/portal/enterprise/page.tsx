...
export default function PortalEnterprisePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink-0">Enterprise domains</h1>
      ...
      <div className="rounded-xl border border-white/10 bg-ink-800 p-4 text-sm text-ink-200">
        <p>
          <span className="text-ink-0">Primary:</span> {primary ?? "—"}
        </p>
        {extras.length > 0 && (
          <p className="mt-2">
            <span className="text-ink-0">Additional monitored:</span> {extras.join(", ")}
          </p>
        )}
        {extras.length === 0 && (
          <p className="mt-2 text-ink-0">No additional domains configured yet.</p>
        )}
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="w-full min-w-[520px] text-left text-sm">
            <thead className="border-b border-white/10 bg-ink-900 text-xs uppercase tracking-wide text-ink-200">
              <tr>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-white/10 hover:bg-ink-800 shadow-sm">
                  <td className="px-3 py-2 font-medium text-ink-0">{r.domain}</td>
                  <td className="px-3 py-2 text-ink-100">{r.hawk_score ?? "—"}</td>
                  <td className="px-3 py-2 capitalize text-ink-200">{r.grade ?? "—"}</td>
                  <td className="px-3 py-2 text-ink-200">{new Date(r.created_at).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
