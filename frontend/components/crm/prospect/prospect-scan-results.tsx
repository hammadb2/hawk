...
function objectionTemplates(industry: string | null): { objection: string; response: string }[] {
  ...
  if (isHealth) {
    return [
      ...
      { objection: "This sounds expensive for what we get.", response: "Compare the cost of one HIPAA incident: OCR breach notification, downtime, and reputation damage typically dwarf preventive monitoring. HAWK Core is $249 per month and includes a $250K Breach Response Guarantee." },
    ];
  }
  if (isLegal) {
    return [
