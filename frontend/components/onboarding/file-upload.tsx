    ...
    <div className="space-y-3">
      <div
        ...
        className={`flex flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition ${
          dragging ? "border-emerald-500 bg-emerald-500/5" : "border-slate-700 bg-[#161625]"
        }`}
      >
        <svg className="mb-3 h-8 w-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
        <p className="text-sm text-slate-400">Drag &amp; drop your government ID here</p>
        <p className="mt-1 text-xs text-slate-600">JPG, PNG, or PDF — max 10 MB</p>
        <label className="mt-4 cursor-pointer rounded-lg bg-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-600 transition">
          Browse files
          <input type="file" accept={ACCEPTED} onChange={onFileSelect} className="hidden" />
        </label>
      </div>
      {fileName && <p className="text-xs text-emerald-400">Selected: {fileName}</p>}
